    ...
    const payload: FormTemplateUpsertDto = {
      ...
      fields: sections.flatMap((s, _sIndex) => 
        s.fields.map((f, fIndex) => ({
          ...
          active: true,
          optionsJson: f.fieldType === 4 && f.optionsJson ? JSON.stringify(f.optionsJson.split(',').map(x => ({ Value: x.trim(), Text: x.trim() }))) : undefined,
                        ...
                        <div key={section.id} className="bg-white rounded-xl border border-surface-muted shadow-sm overflow-hidden flex flex-col transition-all hover:border-brand-primary/40 group">
                            ...
                            <div className="bg-surface-hover px-4 py-3 border-b border-surface-muted flex items-center justify-between gap-4">
                                <div className="flex items-center gap-3 flex-1">
                                    <GripVertical className="h-5 w-5 text-brand-gray cursor-move opacity-50 group-hover:opacity-100 transition-opacity" />
                                        ...
                                        <table className="w-full text-sm text-left">
                                            ...
                                            <tbody className="divide-y divide-surface-muted">
                                                {section.fields.map((f, fIdx) => (
                                                    <tr key={f.id} className="hover:bg-brand-primary/5 transition-colors group/row">
                                                        <td className="px-4 py-2 text-center font-bold text-brand-gray/50">{fIdx + 1}</td>
                        ...
                        sections.map(sec => (
                            <div key={sec.id} className="relative">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-12 gap-5">
                                    {sec.fields.map(f => (
                                        <div key={f.id} className={f.fieldType === 2 ? 'md:col-span-12' : 'md:col-span-6'}>
